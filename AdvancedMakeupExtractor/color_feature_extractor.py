...
class ColorFeature:
    def __init__(self,
        ...
        self.feature_extractor = ColorsFeatureExtractor(self.parser)

    def extract(self,face_img):

        # 颜色特征提取
        color_features = self.feature_extractor(face_img)

        # 结构化输出
        return self._format_features(color_features)

    def _format_features(self, features):
        """将numpy数组转换为字典结构"""
        return {
            "skin_color": features[0:3].tolist(),
            "hair_color": features[3:6].tolist(),
            "lips_color": features[6:9].tolist(),
            "eyes_color": features[9:12].tolist()
        }
