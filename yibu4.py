...
# 回调函数
@celery.task(name='tutorial_callback')
def tutorial_callback(result):
    logger.info(f"Tutorial task completed with result: {result}")
    return result

@celery.task(name='describe_callback')
def describe_callback(result):
    logger.info(f"Describe task completed with result: {result}")
    return result

# 在模块顶层定义回调任务（不要嵌套）
@celery.task(name='process_results')
def process_results(results):
    tutorial_result = next((r for r in results if r.get('type') == 'tutorial'), None)
    describe_result = next((r for r in results if r.get('type') == 'describe'), None)

    tutorial_filename = tutorial_result.get('filename') if tutorial_result and 'filename' in tutorial_result else None
    describe_filename = describe_result.get('filename') if describe_result and 'filename' in describe_result else None

    return {
        'tutorial': tutorial_filename,
        'describe': describe_filename,
        'status': 'completed'
    }
...
def async_orchestrator(self, makeup_path, filename, file_uuid, time_str):
    try:
        ...
        # 启动任务组并设置回调
        chord(task_group)(process_results.s()).apply_async()
