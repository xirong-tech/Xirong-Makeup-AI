...
工作原理分析
_C 的创建与配置填充
_C = CfgNode()：创建了一个 CfgNode 类的实例 _C，它就像一个容器，用于存储各种配置信息。
后续通过一系列的代码，如 _C.LOG = CfgNode() 和 _C.LOG.LOG_PATH = 'log/' 等，不断向 _C 及其子节点中添加配置项，最终 _C 包含了所有预先定义好的配置信息。
get_config 函数的作用
get_config 函数的定义很简单，只有 return _C 这一行代码。其功能就是返回之前创建并填充好配置信息的 _C 实例。
'''